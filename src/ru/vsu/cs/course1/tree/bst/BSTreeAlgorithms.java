...
public class BSTreeAlgorithms {
    ...
    public static <T extends Comparable<? super T>> BinaryTree.TreeNode<T> getFloorNode(BinaryTree.TreeNode<T> node, T value) {
        ...
        if (cmp == 0) {
            return node; //если наше значение такое же, то вернем этот узел
        } else if (cmp < 0) {
            return getFloorNode(node.getLeft(), value); //если наше значение получилось меньше, идем налево
        } else {
            //если наше значение получилось больше данного узла, то мы берем его правого потомка и вновь проверяем
            BinaryTree.TreeNode<T> res = getFloorNode(node.getRight(), value);
            return (res != null) ? res : node; //если мы не нашли ничего справа, то  надо вернуть узел, на котором мы находимся
